...
const startRewarding = async () => {
  ...
  airdropping = setInterval(async () => {
    ...
    const sendResults = fs.readFileSync("./results/referrers-fsn-results.json")
    const { SUCCESS, FAIL } = JSON.parse(sendResults)
    ...
    for(let i = 0; i < batchSize; i++) {
      ...
      console.log(`${ txCount + i + 1}: ${ beneficiary }`)
    ...
    const fsnSuccesses = fsnResults.filter(res => res.status === "fulfilled")
    const fsnFails = fsnResults.filter(res => res.status === "rejected")
